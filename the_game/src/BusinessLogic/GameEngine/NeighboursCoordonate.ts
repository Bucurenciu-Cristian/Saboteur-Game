...
export function neighboursCoordonate(matrix: IMatrix[][], row: number, column: number, occupied = true) {
  const neighbours = [];
  const directions = [
    { name: conDirections.NORTH, coordinate: { row: row - 1, column }, Occupied: occupied },
    { name: conDirections.EAST, coordinate: { row, column: column + 1 }, Occupied: occupied },
    { name: conDirections.SOUTH, coordinate: { row: row + 1, column }, Occupied: occupied },
    { name: conDirections.WEST, coordinate: { row, column: column - 1 }, Occupied: occupied },
  ];
